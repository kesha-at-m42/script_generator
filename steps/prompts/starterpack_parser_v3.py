# ── V3 / Phase 8 skeleton ────────────────────────────────────────────────────
# Derived from: starterpack_parser.py
# For use with: *_generator_v3 pipelines (unit100+ Phase 8 skeleton format)
# ─────────────────────────────────────────────────────────────────────────────
"""
starterpack_parser_v3 - AI Prompt

Converts a single pre-split spec section into a structured JSON object.
Runs in batch mode — one Claude call per section produced by spec_splitter_v3.

Input (user message via batch):
    <input> - {index, major, minor, header, body} from spec_splitter_v3
...
STARTERPACK_PARSER_V3_PROMPT = Prompt(
