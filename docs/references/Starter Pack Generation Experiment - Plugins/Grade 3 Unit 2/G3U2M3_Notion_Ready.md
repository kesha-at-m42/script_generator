...
<!-- HUB PROPERTIES (set manually after import):
  module_id: M03
  module_title: Structured Counting — Rows and Columns
  domain: measurement_area
  unit: 2
  primary_toys: Grid Rectangles
  secondary_toys: none
...
**Key Function:** This Warmup positions M3 as the answer to a question M2 left open. Students arrive knowing how to tile rectangles perfectly and count every tile (M2 skill). The Warmup activates that skill (W.1), then reframes it as a problem (W.2) — counting every tile is accurate but slow. The bridge creates the motivating question ("Is there a faster way?") that the Lesson will answer. The 6×4 rectangle is deliberately chosen as the module's narrative anchor: it returns in Synthesis for the skip-counting payoff.

**Why this serves the concept:**
* M3's new learning is structural seeing (rows/columns) and efficient counting (skip-counting) — perceptual and strategic, not procedural
* Students need to FEEL the problem before learning the solution — the tedium of counting 24 tiles one by one IS the motivation
* Unlike modules where the Warmup activates a skill the Lesson extends, M3's Warmup activates a skill the Lesson REPLACES — counting one by one is what students do BEFORE they see structure
* The 6×4 rectangle establishes the narrative thread: count it the slow way now, skip-count it the fast way in Synthesis

**Test:** If we removed this Warmup, would students lose mathematical preparation for the Lesson? YES — they would enter the Lesson without feeling why structure matters. The first worked example (1.1) shows rows lighting up on a rectangle, and the Guide says "instead of counting all 15 tiles, I can count faster." Without W.1's tedious counting experience, "faster" has no felt referent — it's an abstract claim instead of a personal relief.
...
### Interaction W.1: Count the Tiles
...
### Interaction W.2: That Took a While
...
### Interaction 1.1: Watch the Rows Light Up [Worked Example #1 — Full]
...
### Interaction 1.2: How Many in Each Line?
...
### Interaction 1.3: Now You Name It — "Rows"
...
### Interaction 1.4: How Many Rows? [Worked Example #2 — Partial]
...
### Interaction 1.5: Your Turn to Skip-Count
...
### Interaction 2.1: Now Watch the Other Direction [Worked Example #3 — Partial]
...
### Interaction 2.2: Columns — The Name
...
### Interaction 2.3: Describe It Both Ways
...
### Interaction 2.4: Skip-Count Both Ways
...
### Interaction 2.5: Array — You've Seen This Before
...
### Interaction 3.1: No Highlighting — You See It
...
### Interaction 3.2: Describe It Your Way
...
### Interaction 3.3: Bigger Rectangle
...
### EC Problem 1: Identify the Structure [IDENTIFY]
...
### EC Problem 2: Describe the Structure [CREATE]
...
### EC Problem 3: Skip-Count to Find Area [IDENTIFY]
...
### EC Problem 4: Find the Area Using Columns [IDENTIFY]
...
| Use Grid Rectangles (Display) with visual support | Use highlighting as scaffold (S.1 system animation is demonstration, not scaffold — see KDD 10) |
...
### Connection Task S.1: Two Ways to See It
...
### Connection Task S.2: Rows and Columns Are Everywhere
...
### Metacognitive Reflection S.3: What Helped You Most?
...
KDD documents the pedagogical design choices that shape how this module works — the decisions a writer, reviewer, or future author needs to understand to preserve M3's instructional integrity. Each entry explains WHAT was decided and WHY it matters for student learning.

---

### KDD 1: Structure Before Area — Always
Students must identify row/column structure BEFORE calculating area in every interaction. This is M3's single most important design principle. If students produce correct areas through one-by-one counting without ever seeing structure, M3 has failed — M4's multiplication bridge requires structural understanding, not just correct totals. The four-problem EC design enforces this by requiring structure identification as a separate, assessed step before any area question.

### KDD 2: Three-Step Highlighting IS the Instructional Strategy
The system auto-highlight → student tap-to-highlight → no highlighting progression isn't scaffolding in the usual sense — it IS how M3 teaches structure-seeing. For the 69% of students who cannot spatially structure arrays without instruction, the jump from "system shows you" directly to "find it yourself" skips the critical transfer step. Tap-to-highlight is student-INITIATED but system-EXECUTED — the active practice step where internalization happens. Removing the middle step breaks the module's instructional core.

### KDD 3: Vocabulary After Grounding
Students see "lines going across" before hearing "rows," see "groups going up and down" before hearing "columns," and practice both-direction descriptions before hearing "array." Every vocabulary term enters AFTER students have concrete experience with the concept it names. This isn't just sequencing preference — it's the vocabulary-after-grounding principle that prevents students from learning labels without understanding.

### KDD 4: Same Rectangle, Different Direction
The 2×5 rectangle appears first for row discovery (1.5) and returns immediately for column discovery (2.1). Both skip-counts are trivially easy (by 5s and by 2s), so the ONLY thing landing is the insight: same rectangle, different direction, same area. This is the most important pedagogical moment in Section 2 — the rectangle was chosen specifically so that arithmetic contributes zero cognitive load during this revelation.

### KDD 5: "Array" as Capstone, Not Mid-Section
"Array" vocabulary moves to the end of Section 2 (interaction 2.5), after students have discovered columns, named them, and practiced both-direction descriptions with tap-to-highlight. The arc is: see it → name it → practice it → name the whole thing. Introducing "array" mid-section would stack three new vocabulary terms (column + array + structure-description language) in a single cognitive window.

### KDD 6: Skip-Counting as Demonstration, Not Student Operation
Skip-counting in Sections 1-2 is narrated by the Guide and accompanied by system-driven sequential highlighting. Students observe the pattern; they don't operate the highlighting during counting. The student's cognitive work is seeing structure and answering structure/area questions — the system handles the counting animation. This isolates the structure-seeing demand from the counting-procedure demand.

### KDD 7: Column Flexibility as Capstone Skill
EC.4 tests whether a student can describe a row-highlighted rectangle using COLUMNS — the hardest cognitive demand in M3. It's placed last in EC deliberately. A student who can look at highlighted rows and produce a column description has fully internalized that structure exists in both directions, independent of how it's visually presented. This is the gatekeeper skill for M4.

### KDD 8: No Multiplication Preview
M3 ends on the skip-counting achievement. The multiplication connection ("6 rows of 4 = 24, and 6 × 4 = 24") belongs entirely in M4's warmup. Previewing multiplication in M3 Synthesis would undermine M3's own pedagogical accomplishment — students should leave feeling that structured counting is powerful in its own right, not merely a stepping stone to something better. M3's closure says "keeps getting more useful" without naming the specific application.

### KDD 9: Warmup 6×4 as Narrative Anchor
The 6×4 rectangle threads the entire module: Warmup (tedious one-by-one counting), Synthesis S.1 (system animates rows then columns, both yielding 24), and Synthesis S.4 (student skip-counts it independently). The "6" is outside the Lesson dimension constraint {2, 3, 4, 5, 10}, but the skip-count increment is 4 (columns), which IS Grade 2 prior knowledge. The narrative payoff — returning to the Warmup rectangle with a new strategy — is worth the constraint exception.

### KDD 10: Synthesis Animation as Demonstration, Not Scaffold
S.1 uses system-driven highlighting despite the general Synthesis principle of "no scaffolding." The animation is a DEMONSTRATION tool — making the "both directions yield 24" equivalence visible — not a scaffold. Students have already demonstrated mastery in EC and Practice. The consolidation insight is experiential: it lands through watching the animation, not through answering a question. This is the module's narrative climax.

### KDD 11: Dual-Correct MC in Section 3
Interaction 3.2 accepts both "5 rows of 4" and "4 columns of 5" as correct. The entire point of Section 3 is that either description is valid. Penalizing a correct column answer when the prompt says "your choice" would undermine the insight Section 2 built. Engineering must support dual-correct MC; fallback is row-based primary with column-based flagged as alternative correct.

### KDD 12: Four EC Problems, CREATE and IDENTIFY Only
TVP specifies 4 EC problems testing 4 distinct skills: (1) row structure + area, (2) structure description construction + area, (3) skip-counting validation, (4) column flexibility. COMPARE is deferred to Practice (15% Area Comparison target) per the EC Playbook M1-3 guideline, even though Lesson Section 2 teaches comparison. EC.4's column flexibility test implicitly confirms the "both directions = same area" insight.

### KDD 13: "Array" as Callback from Unit 1
"Array" is staged as a callback, not a new introduction. Students who completed Unit 1 already encountered the term; the M3 formalization ("rows and columns together — that's called an array") activates prior knowledge for those students and teaches it through grounding for those who missed it. Either way, the term enters after students have concrete experience with the row-column structure it names.

### KDD 14: MC Default, Palette Entry Ready
All area answer interactions use MC. Palette entry (broader numeric selector) may replace MC in future — SP interactions are designed so only the input mechanism changes, not the interaction structure. MC is the safest default for initial implementation; the "process of elimination" risk on 4-option MC is acceptable given that structure identification (not area calculation) is the assessed skill in most interactions.

### KDD 15: EC.3 Rectangle — 10×2 for Visible Skip-Counting
10×2 (area 20) chosen over 4×3 (area 12, which matched EC.1). Author requested distinct areas across all EC problems. 10×2's tall orientation makes the skip-counting process highly visible — 10 steps of counting by 2s. It's a rotation of Lesson's 2×10 but a genuinely different rectangle (different orientation, different row/column counts).

### KDD 16: Section 1 Dimension Relaxation
Section 1 constraint relaxed from "both dimensions from {2, 5, 10}" to "at least one from {2, 5, 10}; other from {2, 3, 5, 10}." Interaction 1.1 uses 3×5 — pedagogically optimal for the first worked example because 3 rows is enough to see the pattern, 5 per row is easy to count, and skip-counting by 5 is trivially easy Grade 2 prior knowledge.

### KDD 17: EC Area Range 6-50
EC area range set at 6-50 (matching Practice), not 10-50. EC.4 tests column flexibility on a 2×4 rectangle (area 8) — the cognitive demand is structural, not computational. A hard 10-50 floor would force every structural-focus EC problem to document an exception.

---

## KDD Verification

- [x] **Pedagogical design choices documented:** Structure-before-area (KDD 1), highlighting strategy (KDD 2), vocabulary sequencing (KDD 3), same-rectangle insight (KDD 4), array placement (KDD 5), skip-count demonstration (KDD 6), column flexibility capstone (KDD 7) ✓
- [x] **Scope and bridge decisions documented:** No M4 teaser (KDD 8), narrative anchor (KDD 9) ✓
- [x] **Playbook departures documented:** Synthesis animation (KDD 10), EC area range (KDD 17), dimension relaxation (KDD 16), 4 EC problems (KDD 12) ✓
- [x] **Engineering considerations documented:** Dual-correct MC (KDD 11), MC/palette flexibility (KDD 14) ✓
- [x] **Vocabulary decisions documented:** Array as callback (KDD 13), vocabulary-after-grounding (KDD 3), array as capstone (KDD 5) ✓
- [x] **KDD covers decisions from ALL phases:** Warmup (KDD 9) ✓, Lesson (KDD 1-7) ✓, EC (KDD 7, 12, 15, 17) ✓, Practice (KDD 14) ✓, Synthesis (KDD 8, 10) ✓
