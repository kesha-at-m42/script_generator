...
def detect_phase(lines: list, line_idx: int) -> tuple:
    ...
    for i in range(line_idx, -1, -1):
        ...
        if re.match(r"^#{1,2}\s+\**§?1\.7\b|^#{1,2}\s+\**LESSON\b", line, re.IGNORECASE):
            ...
            break
        if re.match(
            r"^#{1,2}\s+\**§?1\.8\b(?!\.5)|^#{1,2}\s+\**EXIT\s*CHECK\b", line, re.IGNORECASE
        ):
            phase = "EC"
            break
        if re.match(
            r"^#{1,2}\s+\**§?1\.8\.5\b|^#{1,2}\s+\**PRACTICE\s+INPUTS?\b", line, re.IGNORECASE
        ):
            phase = "Practice"
            break
...
def parse_interactions(lines: list) -> list:
    ...
    for i, line in enumerate(lines):
        ...
        is_bold_interaction = (
            stripped.startswith("**Interaction")
            or stripped.startswith("**Connection Task")
            or stripped.startswith("**Metacognitive")
            or stripped.startswith("**EC Problem")
            or stripped.startswith("**Opening Frame")
            or stripped.startswith("**Identity-Building")
        )
        ...
        if is_heading or is_bold_interaction:
            ...
            if current and is_heading and stripped.startswith("##"):
                ...
                if not any(s in heading_lower for s in SKIP_HEADERS):
                    ...
                    if re.match(
                        r"^#{1,3}\s+\**(?:Section\s+\d|§|1\.\d|BACKBONE|PHASE|Verification|Core Purpose|Lesson Structure|Lesson Req|Pedagogical)",
                        stripped,
                        re.IGNORECASE,
                    ):
                        _finalize_interaction(current)
        ...
        if field_line.startswith("* **Purpose:**"):
            ...
            if text:
                dl = DialogueLine(
                    "On Correct", text, current.id, current.title, current.phase, i + 1
                )
                current.dialogue_lines.append(dl)
            ...
            if text:
                dl = DialogueLine(
                    "On Selection", text, current.id, current.title, current.phase, i + 1
                )
                current.dialogue_lines.append(dl)
        ...
        elif field_line.startswith("* **No student action.**") or field_line.startswith(
            "* **No student action**"
        ):
        ...
        elif field_line.startswith("* **On Complete:**") or field_line.startswith(
            "**On Complete:**"
        ):
        ...
        # (e.g. multi-part interactions with "* **Guide (synced):**")
            ...
            "**Guide" in field_line
            and ":**" in field_line
            and not field_line.startswith("* **Guide:**")
        ):
            text = _extract_quoted_after_colon(field_line)
            if text:
                dl = DialogueLine("Guide", text, current.id, current.title, current.phase, i + 1)
...
def _finalize_interaction(interaction: Interaction):
    ...
    if interaction.has_no_student_action and not (
        interaction.has_student_action or interaction.has_prompt
    ):
...
def parse_toys_from_spec(lines: list) -> list:
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r"^###\s+", stripped):
            ...
            primary_m = re.match(
                r"^(?:Primary\s+Toy|Secondary\s+(?:Toy|Tool))\s*:\s*(.+)", title, re.IGNORECASE
            )
            if primary_m:
...
if __name__ == "__main__":
    ...
    for dl in sp.dialogue_lines[:10]:
        print(
            f"  {dl.phase:10s} | {dl.interaction_id:6s} | {dl.field_type:12s} | {dl.text[:60]}..."
        )
