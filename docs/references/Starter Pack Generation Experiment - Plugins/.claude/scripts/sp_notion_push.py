...
def build_notion_properties(props: dict, module_num: int, title: str) -> dict:
    ...
    if lessons:
        lesson_list = [item.strip() for item in lessons.split(",") if item.strip()]
        if lesson_list:
...
def main():
    parser = argparse.ArgumentParser(description="Convert local SP to Notion page format")
    parser.add_argument("sp_file", help="Path to the local SP markdown file")
    parser.add_argument("--json", action="store_true", help="Output structured JSON")
    parser.add_argument("--content-only", action="store_true", help="Output just the markdown body")
