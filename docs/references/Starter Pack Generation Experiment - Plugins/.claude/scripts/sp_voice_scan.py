...
def check_red_flags(dialogue_lines: list) -> list:
    ...
    word_patterns = [
        (w, re.compile(r"\b" + re.escape(w) + r"\b", re.IGNORECASE)) for w in RED_FLAG_WORDS
    ]
    phrase_patterns = [(p, re.compile(re.escape(p), re.IGNORECASE)) for p in RED_FLAG_PHRASES]
...
FEELING_ASSUMPTION_PATTERNS = [
    ...
    (
        re.compile(r"(?<!\bdo )\bYou think\b(?!\s+about)(?!.*\?)", re.IGNORECASE),
        "thought assumption",
    ),
    ...
    (
        re.compile(
            r"\bYou must be\s+(?:proud|frustrated|confused|excited|happy|nervous|scared|worried|bored|tired)",
            re.IGNORECASE,
        ),
        "feeling assumption",
    ),
    (
        re.compile(
            r"\bYou'?re (?:proud|frustrated|confused|excited|happy|nervous|eager|thrilled)",
            re.IGNORECASE,
        ),
        "feeling assumption",
    ),
...
IDENTITY_LABEL_PATTERNS = [
    (
        re.compile(
            r"\bYou'?re an? (?:mathematician|math expert|expert|genius|star|champion|whiz|pro)\b",
            re.IGNORECASE,
        ),
        "identity label",
    ),
    (
        re.compile(r"\bLike an? (?:mathematician|scientist|expert|real)\b", re.IGNORECASE),
        "identity label",
    ),
    (
        re.compile(r"\bYou'?re (?:so smart|a natural|brilliant|gifted)\b", re.IGNORECASE),
        "identity label",
    ),
    (
        re.compile(r"\bmath (?:wizard|whiz|star|superstar|genius|champion)\b", re.IGNORECASE),
        "identity label",
    ),
...
ACADEMIC_LANGUAGE_PATTERNS = [
    ...
    (
        re.compile(r"\bIsn't that (?:exciting|amazing|incredible|wonderful)\b", re.IGNORECASE),
        "fake enthusiasm",
    ),
...
def run_voice_scan(filepath: str, gate: int) -> dict:
    ...
    all_findings.extend(
        check_exclamation_density(sp_filtered.dialogue_lines, sp_filtered.interactions)
    )
