...
def fetch_node(file_key, node_id):
    # Use the /nodes endpoint — works for any node, not just pages
    ...
    node_data = nodes.get(node_id_colon) or nodes.get(node_id_fmt)
    if not node_data:
...
# Save for reference
with open("figma_experiment/figma_nodes_raw.json", "w") as f:
