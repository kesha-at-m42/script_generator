...
RAW_NODES_PATH = "figma_experiment/figma_nodes_raw.json"
...
# Batch export frame IDs from Figma in chunks (API limit ~25 per request)
CHUNK_SIZE = 25
all_ids = [n["id"] for n in frame_nodes]
image_urls = {}
...
for i in range(0, len(all_ids), CHUNK_SIZE):
    ...
    r.raise_for_status()
    image_urls.update(r.json().get("images", {}))
    print(f"  chunk {i // CHUNK_SIZE + 1}: {len(chunk)} IDs")
