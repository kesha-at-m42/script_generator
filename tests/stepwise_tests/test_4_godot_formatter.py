...
def test_godot_formatter(
    remediation_path,
    output_dir=None,
    limit=None,
    module_number=None,
    metadata_type="SequenceMetadata",
):
    """
    ...
    if limit and limit < num_sequences:
        ...
        response = (
            input(
                f"\nProcess all {num_sequences} sequences? (y/n or Enter for yes, or enter a number): "
            )
            .strip()
            .lower()
        )
        ...
        if response == "n":
            ...
            try:
                ...
                if custom_limit > 0 and custom_limit < num_sequences:
                    ...
                    print(
                        f"⚠️  Requested {custom_limit} but only {num_sequences} available. Processing all."
                    )
    ...
    print(
        "✓ Stripped non-generic error paths. Each sequence will generate exactly 1 Godot sequence.\n"
    )
    ...
    if module_number is None:
        try:
            module_input = input(
                "Enter module number to fetch vocabulary (e.g., 1) or press Enter to skip: "
            ).strip()
            if module_input:
    ...
    for idx, sequence in enumerate(sequences_list, 1):
        ...
        single_sequence_data = {"sequences": [sequence]}
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "Overloaded" in str(e) and attempt < max_retries - 1:
                    print(
                        f"      ⚠️  API overloaded, retrying in {retry_delay} seconds... (attempt {attempt + 1}/{max_retries})"
                    )
        ...
        with open(
            f"{output_dir}/godot_{idx:03d}_problem{sequence.get('problem_id')}_goal{sequence.get('goal_id')}_raw.txt",
            "w",
            encoding="utf-8",
        ) as f:
        ...
        try:
            ...
            # ================================================================
            print("      ⚙️  Correcting metadata fields...")
            ...
            # ================================================================
            print("      ⚙️  Applying BBCode formatting...")
    ...
    godot_data = {"@type": "SequencePool", "sequences": all_godot_sequences}
    ...
    validation_results = {"total_sequences": len(godot_data.get("sequences", [])), "sequences": []}
    ...
    for idx, seq in enumerate(godot_data.get("sequences", []), 1):
        ...
        for step_idx, step in enumerate(steps, 1):
            ...
            if has_workspace:
                ...
                print(
                    f"      Step {step_idx}: Prompt with validator and {len(remediations)} remediation(s) ✓"
                )
                ...
                for rem_idx, rem in enumerate(remediations):
                    if "@type" not in rem:
                        seq_validation["issues"].append(
                            f"Step {step_idx}, Remediation {rem_idx}: Missing @type"
                        )
                    if "id" not in rem or not isinstance(rem["id"], str) or not rem["id"].strip():
                        seq_validation["issues"].append(
                            f"Step {step_idx}, Remediation {rem_idx}: Invalid or missing id"
                        )

                # Check BBCode formatting
                if "text" in prompt and "/" in prompt["text"] and "[fraction" not in prompt["text"]:
                    seq_validation["issues"].append(
                        f"Step {step_idx}: Prompt text may have unformatted fractions"
                    )
                ...
                if "/" in step["dialogue"] and "[fraction" not in step["dialogue"]:
                    seq_validation["issues"].append(
                        f"Step {step_idx}: Dialogue may have unformatted fractions"
                    )
...
def main():
    ...
    parser.add_argument(
        "remediation_path", help="Path to remediation JSON file (from remediation generator)"
    )
    ...
    parser.add_argument(
        "-m", "--module", type=int, help="Module number to fetch vocabulary", default=None
    )
    parser.add_argument(
        "--metadata-type",
        type=str,
        help="Expected @type value for metadata validation",
    ...
    test_godot_formatter(
        args.remediation_path, args.output, args.limit, args.module, args.metadata_type
    )
