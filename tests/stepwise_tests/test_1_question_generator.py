...
def test_question_generator(
    ...
    for idx, goal in enumerate(learning_goals, 1):
        ...
        print(
            f"  Generating variations of example questions across difficulty range {difficulty_level}"
        )
        ...
        questions_response = client.generate(
            questions_prompt, max_tokens=16000, temperature=1.0, prefill=prefill
        )
    ...
    questions_data = {"summary": summary, "questions": all_questions}
    ...
    for idx, q in enumerate(questions_data.get("questions", []), 1):
        q_validation = {"question_id": q.get("question_id"), "issues": []}
        ...
        if "rectangle" not in visual.lower() and "bar" not in visual.lower():
            ...
            print("    ⚠️  Visual context may not follow constraints")
        else:
            print("    ✓ Visual context uses rectangles/bars")
...
def main():
    ...
    parser.add_argument(
        "-m", "--module", type=int, default=1, help="Module number to load (default: 1)"
    )
    parser.add_argument(
        "-n",
        "--num-questions",
        type=int,
        default=8,
        help="Number of questions per goal (default: 8)",
    )
