...
def test_interaction_designer(
    questions_path, output_dir=None, limit=None, module_number=None, path_letter=None
):
    """
    ...
    if limit is not None:
        ...
        try:
            user_input = input(
                "How many questions would you like to process? (press Enter for all): "
            ).strip()
            if user_input == "":
    ...
    try:
        ...
        for idx, seq in enumerate(sequences_data.get("sequences", []), 1):
            ...
            steps_with_interaction = [
                step for step in seq.get("steps", []) if "interaction_tool" in step
            ]
            steps_with_answer = [step for step in seq.get("steps", []) if "correct_answer" in step]

            print(
                f"    - Steps with workspace: {len(steps_with_workspace)} {'✓' if len(steps_with_workspace) > 0 else '✗'}"
            )
            if len(steps_with_workspace) == 0:
                seq_validation["issues"].append("No steps with workspace field")

            print(
                f"    - Steps with interaction_tool: {len(steps_with_interaction)} {'✓' if len(steps_with_interaction) > 0 else '✗'}"
            )
            if len(steps_with_interaction) == 0:
                seq_validation["issues"].append("No steps with interaction_tool")

            print(
                f"    - Steps with correct_answer: {len(steps_with_answer)} {'✓' if len(steps_with_answer) > 0 else '✗'}"
            )
            ...
            print(
                f"    - Steps with visual field: {steps_with_visual}/{seq_validation['num_steps']} {'✓ (should be 0)' if steps_with_visual == 0 else '⚠️'}"
            )
            if steps_with_visual > 0:
                seq_validation["issues"].append(
                    "Visual field present in main flow steps (should be omitted)"
                )
            ...
            error_paths = [
                k for k in seq.get("student_attempts", {}).keys() if k.startswith("error_path")
            ]
            has_error_paths = len(error_paths) > 0
            print(
                f"    - Has error paths: {len(error_paths)} {'✗ (should be 0)' if has_error_paths else '✓'}"
            )
            if has_error_paths:
        ...
        print("\n📊 Expected Schema:")
        print(
            "  ✓ Steps should have: dialogue, prompt, interaction_tool, workspace, correct_answer"
        )
