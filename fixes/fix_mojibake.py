...
REPLACEMENTS = [
    ...
    (
        "\u00e2\u20ac\u201d",
        "\u2014",
    ),  # â€"  → —  (U+2014 em dash;   UTF-8: E2 80 94; cp1252: E2=â, 80=€, 94=")
    (
        "\u00e2\u20ac\u201c",
        "\u2013",
    ),  # â€"  → –  (U+2013 en dash;   UTF-8: E2 80 93; cp1252: E2=â, 80=€, 93=")
    (
        "\u00e2\u2020\u2019",
        "\u2192",
    ),  # â†'  → →  (U+2192 r-arrow;  UTF-8: E2 86 92; cp1252: E2=â, 86=†, 92=')
    (
        "\u00e2\u20ac\u2122",
        "\u2019",
    ),  # â€™  → '  (U+2019 r-s-quote; UTF-8: E2 80 99; cp1252: E2=â, 80=€, 99=™)
    (
        "\u00e2\u20ac\u0153",
        "\u201c",
    ),  # â€œ  → "  (U+201C l-d-quote; UTF-8: E2 80 9C; cp1252: E2=â, 80=€, 9C=œ)
    (
        "\u00e2\u20ac\u009d",
        "\u201d",
    ),  # â€   → "  (U+201D r-d-quote; UTF-8: E2 80 9D; cp1252: E2=â, 80=€, 9D=undef→use raw)
    ("\u00e2\u20ac\u2022", "\u2022"),  # â€¢  → •  (U+2022 bullet;    UTF-8: E2 80 95; cp1252: 95=•)
    (
        "\u00e2\u20ac\u2026",
        "\u2026",
    ),  # â€¦  → …  (U+2026 ellipsis;  UTF-8: E2 80 A6; cp1252: A6=... mapped)
    ("\u00c3\u00a9", "\u00e9"),  # Ã©   → é  (U+00E9)
    ("\u00c3\u00a0", "\u00e0"),  # Ã    → à  (U+00E0)
    ("\u00c3\u00b3", "\u00f3"),  # Ã³   → ó  (U+00F3)
]
...
def fix_module(unit: int, module: int, dry_run: bool = False) -> None:
    ...
    for filename in PHASE_FILES:
        ...
        if original == fixed:
            ...
            counts = {
                label: original.count(bad)
                for bad, _ in REPLACEMENTS
                for label in [repr(bad)]
                if original.count(bad)
            }
