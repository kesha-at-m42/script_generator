"""
fix_10013_math_expression.py

For all sequences with template_id 10013:
  - Shuffle the fraction order from metadata identifiers
  - Build a MathExpression with terms interleaved with " " spacers
  - Append it to workspace tangibles (creating workspace if needed)

Usage:
    python fix_10013_math_expression.py <input_file> [output_file]
...
def process(data: dict | list) -> int:
    ...
    for seq in sequences:
        ...
        if str(meta.get("template_id")) != "10013":
            continue
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python fix_10013_math_expression.py <input_file> [output_file]")
        sys.exit(1)
    ...
    print(f"Modified {modified} step(s) across 10013 sequences")
