...
class QAFilter:
    ...
    def validate_and_filter(
        ...
        passed_system = [r for r in system_results["results"] if r["is_valid"]]
        ...
        if not passed_system:
            if self.verbose:
                print("  ⚠️  No items passed system validation!")
            return [], {
                "total": len(items),
                "filtered": 0,
                "removed": len(items),
        ...
        ai_results = self.ai_validator.validate_batch(ai_items, validation_type, self.module_data)
        ...
        for sys_result in passed_system:
            ...
            ai_result = next((r for r in ai_results["results"] if r["item"] == item), None)
                ...
                scored_items.append(
                    {
                        ...
                        and composite_score >= min_score_threshold,
                    }
                )
        ...
        if filter_strategy == "top_percent":
            ...
            filtered_items = [
                item for item in scored_items if item["composite_score"] >= filter_value
            ]
        else:
        ...
        if self.verbose:
            ...
            if filtered_items:
                avg_score = sum(item["composite_score"] for item in filtered_items) / len(
                    filtered_items
                )
                print(f"    Avg score of kept items: {avg_score:.1f}/100")
                print(
                    f"    Score range: {filtered_items[-1]['composite_score']:.1f} - {filtered_items[0]['composite_score']:.1f}"
                )
    ...
    def save_report(self, report: Dict, output_path: Path):
        ...
        simplified_report = {
            ...
            if report["ai_validation"]
            else None,
            "item_scores": [
                {
                    "item_id": item.get("item", {}).get("question_id")
                    or item.get("item", {}).get("sequence_id", "unknown"),
