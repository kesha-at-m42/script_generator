...
class SystemValidator:
    ...
    def validate_question(
        self, question: dict, module_data: dict = None
    ...
    def validate_interaction(
        self, sequence: dict, module_data: dict = None
    ) -> Tuple[bool, float, List[str]]:
        """
            ...
            for idx, interaction in enumerate(interactions):
                ...
                if interaction_type in ["select", "multi_select", "click"]:
                    ...
                    if not choices or len(choices) < 2:
                        issues.append(
                            f"Interaction {idx}: {interaction_type} must have at least 2 choices"
                        )
                        score -= 15
                    ...
                    for choice_idx, choice in enumerate(choices):
                        if not isinstance(choice, dict):
                            issues.append(
                                f"Interaction {idx}, choice {choice_idx}: Invalid choice format"
                            )
                            score -= 5
                            continue
                        ...
                        if "text" not in choice or not choice["text"]:
                            issues.append(
                                f"Interaction {idx}, choice {choice_idx}: Missing choice text"
                            )
                            score -= 5

                        if "correct" not in choice:
                            issues.append(
                                f"Interaction {idx}, choice {choice_idx}: Missing 'correct' field"
                            )
                            score -= 5
    ...
    def validate_batch(
        self, items: List[dict], validation_type: str, module_data: dict = None
    ) -> Dict:
        """
        ...
        for item in items:
            ...
            results.append({"item": item, "is_valid": is_valid, "score": score, "issues": issues})
